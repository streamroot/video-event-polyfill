...
class VideoEventPolyfill extends EventEmitter {
  ...
  constructor (video) {
    ...
    this.onSking = this.onSeeking.bind(this);
    this.onCpl = this.onCanplay.bind(this);
    this.onCplt = this.onCanplaythrough.bind(this);
  ...
  start () {
    ...
    this.video.addEventListener('seeking', this.onSking);
  }
  ...
  onSeeking () {
    if (this.video.paused) {
      this.state = this.SEEKING_WHEN_PAUSED;

      this.video.removeEventListener('playing', this.onPling); // remove previous playing event listeners if it exist
      this.video.removeEventListener('canplay', this.onCpl);
      this.video.removeEventListener('canplaythrough', this.onCplt);
      clearTimeout(this._canplayTimeout);

      this.video.addEventListener('playing', this.onPling);
      this.video.addEventListener('canplay', this.onCpl);
      this.video.addEventListener('canplaythrough', this.onCplt);
    }
  }

  onCanplay () {
    /*
     * The canplaythrough workaround (see comments in onCanplaythrough below) might not always work. Actually, it doesn't in FF OSX where the canplaythrough event is not triggered
     * In that case we set a timeout when we receive the canplay event. If we don't receive a canplaythrough event in that timeframe, we call play when the timeout expires.
     */

    this._canplayTimeout = setTimeout(() => {
      if (this.state === this.SEEKING_WHEN_PAUSED) {
        // If state is SEEKING_WHEN_PAUSED, we still have the auto-removing onPlaying listener we set in onSeeking.
        // Play will trigger a video 'playing' event, which will set the state to PLAYING

        this.video.removeEventListener('canplaythrough', this.onCplt); // remove canplaythrough listener first as we perform the workaround here
        this.video.play();
      }
    }, 500);
  ...
  onCanplaythrough () {
    /*
     * When seek is complete, the video tag emits:
     * 1) seeked
     * 2) canplay
     * 3) playing
     * 4) canplaythrough
     *
     * If we didn't receive the playing event, then JW won't call play and the video will be stuck. Call play manually.
     * This is a temporary hack for our JW integration ONLY (this shouldn't land in master !!!)
     */
    clearTimeout(this._canplayTimeout);
  ...
  tick () {
    ...
    switch(this.state) {
      ...
      case this.SEEKING_WHEN_PAUSED: // Do nothing if seeking, we wait for the canplay through event to trigger a playing event
  ...
  dispose () {
    ...
    if (this.video) {
      this.video.removeEventListener('play', this.onPl);
      this.video.removeEventListener('playing', this.onPling);
      this.video.removeEventListener('seeking', this.onSking);
      this.video.removeEventListener('canplaythrough', this.onCplt);
      delete this.video;
    }
